...
import { generateAgeProofSigned, generateNationalityProofSigned } from '../src/prover';
import {
  loadVerificationKey,
...
describe('Signed Proof Integration', () => {
  ...
  it('should generate and verify a signed nationality proof end-to-end', async function () {
    this.timeout(60000);
  ...
  it('should fail verification with wrong issuer public key', async function () {
    this.timeout(60000);
    ...
    const wrongIssuer = await CircuitCredentialIssuer.createTestIssuer('Wrong Issuer');
